...
/**
 * Line-oriented TCP/TLS transport shared by [ImapClient] and [SmtpClient].
 *
 * IMAP and SMTP are line protocols; the only transport-level subtlety is STARTTLS,
 * which re-handshakes the existing socket in place ([upgradeToTls]). All exceptions are
 * built from server responses or connection state, never from the sent command text,
 * so credentials cannot leak through error surfaces.
 */
...
suspend fun createEmailConnection(host: String, port: Int, tls: Boolean): EmailConnection = withContext(Dispatchers.IO) {
    ...
    SocketEmailConnection(socket)
}

private class SocketEmailConnection(
