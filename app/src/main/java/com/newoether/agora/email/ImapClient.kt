...
/**
 * Email content as fetched from IMAP, before persistence mapping.
 * [dateRaw] is the unparsed RFC 5322 date; [dateEpochMs] its best-effort parse (0 when unknown).
 */
data class EmailFetchedMessage(
    val uid: Long,
 ...
 * tagged commands (e.g. "A001 LOGIN ..."), SELECT, SEARCH, FETCH with BODY.PEEK,
 * STORE flags, LIST with SPECIAL-USE, CREATE, and APPEND with literals.
 *
 * Security invariant: exceptions are built only from server responses or connection
 * state, never from the sent command text — the LOGIN command carries the password.
 * [connectionFactory] is injectable so JVM tests can script a fake server without
 * opening sockets.
 */
class ImapClient(
    ...
    suspend fun connect() {
        connection = connectionFactory(host, port, tls)
     ...
     */
    suspend fun fetchHeaders(uids: List<Long>, accountId: String): List<EmailFetchedMessage> {
        if (uids.isEmpty()) return emptyList()
        val conn = connection ?: throw IllegalStateException("Not connected")
        val messages = mutableListOf<EmailFetchedMessage>()
     ...
     */
    suspend fun fetchBody(uid: Long, accountId: String): EmailFetchedMessage? {
        val conn = connection ?: throw IllegalStateException("Not connected")
        val tag = nextTag()
    ...
    private fun parseEmailFromFetch(uid: Long, accountId: String, raw: String): EmailFetchedMessage? {
        var from = ""
        var to = ""
        var subject = ""
        var date = ""
        ...
        return EmailFetchedMessage(
            ...
            subject = subject,
            dateRaw = date,
            dateEpochMs = parseRfc5322DateToEpochMs(date),
            preview = preview,
            body = body,
            bodyHtml = bodyHtml,
            messageId = messageId,
    ...
    private fun detectMimeBoundary(content: String): String? =
        mimeBoundaryRegex.find(content)?.groupValues?.get(1)
    ...
    // Entities are built via concatenation so the source stays literal ("&" + "amp;"),
    // not because of runtime needs.
...
/**
 * Best-effort RFC 5322 date parse ("Mon, 21 Sep 2026 10:00:00 +0000" and close
 * variants). Returns 0 when the date can't be understood — callers treat 0 as
 * "unknown date" and never reorder delivery by it (delivery order is the UID).
 */
internal fun parseRfc5322DateToEpochMs(date: String): Long {
    val trimmed = date.trim().removeSuffix("(GMT)").trim()
    ...
    for (pattern in patterns) {
        try {
            return OffsetDateTime.parse(trimmed, pattern).toInstant().toEpochMilli()
        } catch (_: Exception) {
        }
    }
    return 0L
